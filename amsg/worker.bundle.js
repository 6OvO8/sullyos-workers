...
// node_modules/.pnpm/@rei-standard+amsg-server@2.6.0-next.5/node_modules/@rei-standard/amsg-server/dist/chunk-ES2HUXYZ.mjs
...
function buildXhsSessionPayload(directives, notes, xsecTokens) {
  ...
  for (const idx of [...sharedIdx].sort((a, b) => a - b)) {
    ...
    if (!note) continue;
    if (pickedNotes.length >= XHS_NOTES_MAX) {
      droppedForCap++;
      continue;
    }
  ...
  if (droppedForCap > 0) {
    console.warn(
      "[amsg:agentic] XHS \u7B14\u8BB0\u8D85\u8FC7\u5355\u6761 push \u4E0A\u9650\uFF0C\u5DF2\u4E22\u5F03",
      { cap: XHS_NOTES_MAX, dropped: droppedForCap }
    );
  }
...
var buildToolCtx = (toolPackRaw, toolConfigRaw, taskId, charId) => {
  const pack = parseToolPack(toolPackRaw);
  if (!pack) throw fireStateError("tool_pack \u89E3\u6790\u5931\u8D25\uFF08\u683C\u5F0F\u4E0D\u5BF9\u6216\u6570\u636E\u635F\u574F\uFF09", { taskId, charId });
  const config = parseToolConfig(toolConfigRaw);
  if (!config) throw fireStateError("tool_config \u89E3\u6790\u5931\u8D25\uFF08\u683C\u5F0F\u4E0D\u5BF9\u6216\u6570\u636E\u635F\u574F\uFF09", { taskId, charId });
  const char = {
    name: pack.charName,
...
var amsgHooks = {
  async onBeforeFire(ctx) {
    ...
    if (!packRow) {
      throw fireStateError("\u4E91\u7AEF\u6CA1\u6709\u8FD9\u4E2A\u89D2\u8272\u7684 fire_pack", { taskId: ctx.task.id, charId });
    }
    const pack = parseFirePack(packRow.value);
    if (!pack) {
      throw fireStateError("fire_pack \u89E3\u6790\u5931\u8D25\uFF08\u683C\u5F0F\u4E0D\u5BF9\u6216\u6570\u636E\u635F\u574F\uFF09", { taskId: ctx.task.id, charId });
    }
    const occurrenceMs = Date.parse(String(ctx.task.nextSendAt));
    if (!Number.isFinite(occurrenceMs)) {
      throw fireStateError("\u4EFB\u52A1\u884C next_send_at \u89E3\u6790\u4E0D\u51FA\u89E6\u53D1\u65F6\u523B", {
        taskId: ctx.task.id,
        charId,
        nextSendAt: ctx.task.nextSendAt
      });
    }
    const expireInput = {
      policy,
    ...
    if (typeof taskMeta.amsgTaskInstruction !== "string") {
      throw fireStateError("\u4EFB\u52A1 metadata \u7F3A amsgTaskInstruction\uFF08\u65E7\u683C\u5F0F\u4EFB\u52A1\uFF09", {
        taskId: ctx.task.id,
        charId
      });
    }
    ...
    if (!toolPackRow) {
      throw fireStateError("\u4E91\u7AEF\u6CA1\u6709\u8FD9\u4E2A\u89D2\u8272\u7684 tool_pack", { taskId: ctx.task.id, charId });
    }
    if (!toolConfigRow) {
      throw fireStateError("\u4E91\u7AEF\u6CA1\u6709 tool_config", { taskId: ctx.task.id, charId });
    }
